...
class FluxAttnProcessor2_0:
    ...
    def __call__(
        ...
        # print("DiT_mask_shape:",attention_mask.shape)
        # print("DiT_q_shape:",query.shape)
        # print(f"[PROCESSOR] Called. Query sequence length: {query}, attn_mask shape: {attention_mask.shape if attention_mask is not None else 'None'}")
        hidden_states = F.scaled_dot_product_attention(
            query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
        )
        ...
        if return_attnscore:
            ...
            attnscore = query @ key.transpose(-2, -1) * scale_factor
            if encoder_hidden_states is not None:
                encoder_hidden_states, hidden_states = (
                    hidden_states[:, : encoder_hidden_states.shape[1]],
                    hidden_states[:, encoder_hidden_states.shape[1] :],
                ...
                return hidden_states, encoder_hidden_states, attnscore
            else:
                return hidden_states, attnscore
        else:
            if encoder_hidden_states is not None:
                encoder_hidden_states, hidden_states = (
                    hidden_states[:, : encoder_hidden_states.shape[1]],
