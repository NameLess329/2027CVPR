...
class CustomFluxTransformer2DModel(FluxTransformer2DModel):
    ...
    def __init__(
        ...
        self.rcm_head = None
        self.rcm_probe_enabled = False
        self.rcm_probe_include_background = False
        self.rcm_probe_tap_block = "last"
        self.last_rcm_outputs = None
        self.last_rcm_tap_block = None
    ...
    def set_rcm_probe(self, rcm_head=None, enabled: bool = True, include_background: bool = False, tap_block="last"):
        """Attach an optional stage-1 RCM probe without changing generation."""

        self.rcm_head = rcm_head
        self.rcm_probe_enabled = bool(enabled and rcm_head is not None)
        self.rcm_probe_include_background = bool(include_background)
        self.rcm_probe_tap_block = tap_block
        self.last_rcm_outputs = None
        self.last_rcm_tap_block = None

    def _should_tap_rcm_block(self, index_block: int) -> bool:
        tap_block = self.rcm_probe_tap_block
        if tap_block is None or str(tap_block).lower() == "last":
            return False
        try:
            return int(tap_block) == int(index_block)
        except (TypeError, ValueError):
            raise ValueError("rcm_probe tap_block must be 'last' or an integer transformer block index")
    ...
    def forward(
        ...
        rcm_tapped_hidden_states = None
        rcm_tapped_block = None
        for index_block, block in enumerate(self.transformer_blocks):
            ...
            if self.rcm_probe_enabled and self._should_tap_rcm_block(index_block):
                rcm_tapped_hidden_states = hidden_states
                rcm_tapped_block = index_block
        ...
        self.last_rcm_outputs = None
        self.last_rcm_tap_block = None
        if self.rcm_probe_enabled and self.rcm_head is not None:
            if str(self.rcm_probe_tap_block).lower() == "last":
                rcm_source_hidden_states = hidden_states
                self.last_rcm_tap_block = "last"
            elif rcm_tapped_hidden_states is not None:
                rcm_source_hidden_states = rcm_tapped_hidden_states
                self.last_rcm_tap_block = rcm_tapped_block
            else:
                raise ValueError(
                    f"Requested rcm tap_block={self.rcm_probe_tap_block}, "
                    f"but transformer has {len(self.transformer_blocks)} blocks indexed 0..{len(self.transformer_blocks) - 1}."
                )
            if self.rcm_probe_include_background:
                rcm_split_sizes = split_sizes
                rcm_hidden_states = rcm_source_hidden_states[:, l0_len:, :]
                rcm_clean_tokens = clean_tokens
                rcm_layer_boxes = list_layer_box[1:]
                rcm_layer_offset = 1
            else:
                rcm_split_sizes = split_sizes[1:]
                bg_len = split_sizes[0]
                rcm_hidden_states = rcm_source_hidden_states[:, l0_len + bg_len:, :]
                rcm_clean_tokens = clean_tokens[:, bg_len:, :]
                rcm_layer_boxes = list_layer_box[2:]
                rcm_layer_offset = 2
            if rcm_split_sizes:
                self.last_rcm_outputs = self.rcm_head(
                    hidden_states=rcm_hidden_states.detach(),
                    clean_tokens=rcm_clean_tokens.detach(),
                    split_sizes=rcm_split_sizes,
                    list_layer_box=rcm_layer_boxes,
                    temb=temb.detach(),
                    image_size=(height * 8, width * 8),
                    layer_offset=rcm_layer_offset,
                )
